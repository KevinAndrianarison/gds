...
export default function FormRegion() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!regionName.trim()) {
            Notiflix.Report.failure('Erreur', 'Veuillez saisir un nom de région', 'OK');
            return;
        }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
                Notiflix.Report.failure('Erreur', 'Une région avec ce nom existe déjà', 'OK');
            } else {
                Notiflix.Report.failure('Erreur', 'Erreur lors de l\'ajout de la région', 'OK');
            }
        } finally {
    ...
    return (
        <form onSubmit={handleSubmit} className='border-b-2 py-4 border-gray-100 flex justify-between'>
            ...
            <div className='border-l w-[25%] border-gray-100 px-4'>
                ...
                {isLoading ? (
                    <FontAwesomeIcon icon={faSpinner} pulse className='text-lg text-gray-700' />
                ) : (
