...
export default function Profil() {
  ...
  const handlePhotoChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        // Mettre à jour la prévisualisation avec la nouvelle photo
        setPhotoPreview(updatedUser.photo_url ? `${url}/storage/${updatedUser.photo_url}` : null)
        setPhoto(null)
        Notify.success('Photo de profil mise à jour')
      } catch (error) {
        Notify.failure('Erreur lors de la mise à jour de la photo')
        setPhotoPreview(userData.photo_url ? `${url}/storage/${userData.photo_url}` : null)
      } finally {
        nProgress.done()
      }
    }
  ...
  const handleInputBlur = async (field) => {
    ...
    try {
      ...
      nProgress.start()
      const response = await axios.put(`${url}/api/update-user`, {
        field,
      ...
      Notify.failure(error.response?.data?.message || 'Erreur lors de la mise à jour')
    } finally {
      nProgress.done()
    }
  }
    ...
    <div className='p-2 max-w-4xl mx-auto'>
      ...
      <div className='bg-gray-50 rounded-xl p-6  mb-6'>
        <div className='flex justify-between items-center mb-6'>
          <h2 className='text-sm font-semibold uppercase'>Informations personnelles</h2>
          <div className='flex justify-end'>
            <button
              onClick={() => setIsEditingProfile(!isEditingProfile)}
              className={`flex items-center  cursor-pointer gap-2 px-4 py-2 rounded-lg ${isEditingProfile ? 'text-red-400' : 'text-blue-400'}`}
            >
              {isEditingProfile ? (
                <>
                  <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' strokeWidth={1.5} stroke='currentColor' className='w-5 h-5'>
      ...
      <div className='bg-gray-50 rounded-xl p-6 '>
        <div className='flex justify-between items-center mb-6'>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg cursor-pointer ${isChangingPassword ? 'text-red-400' : 'text-blue-400'}`}
          >
            {isChangingPassword ? (
              <>
                <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' strokeWidth={1.5} stroke='currentColor' className='w-5 h-5'>
            ...
            <div className='flex flex-col gap-2'>
              ...
              <div className='relative'>
                <InputOn
                  ...
                  onChange={(value) => setPasswords(prev => ({ ...prev, current_password: value }))}
                  disabled={isLoading}
                />
                <button
                  type='button'
            ...
            <div className='flex flex-col gap-2'>
              ...
              <div className='relative'>
                <InputOn
                  ...
                  onChange={(value) => setPasswords(prev => ({ ...prev, new_password: value }))}
                  disabled={isLoading}
                />
                <button
                  type='button'
            ...
            <div className='flex flex-col gap-2'>
              ...
              <div className='relative'>
                <InputOn
                  ...
                  onChange={(value) => setPasswords(prev => ({ ...prev, confirm_password: value }))}
                  disabled={isLoading}
                />
                <button
                  type='button'
